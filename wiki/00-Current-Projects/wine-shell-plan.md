...
**Phase 2 log (2026-09-20, CORE GREEN):** `BackendDeployer` (prefix-local DLL deploy, validate-all-before-touching, x32 only if syswow64 exists, `next(after:)` cycle DXVK→DXMT→wined3d, D3DMetal never auto-cycled), `GPTKValidator` (DMG `lib`/`redist`/volume locate + framework/forwarders presence + winebuild builtin-marker check, fail-closed), `RecipeRunner` (verbs → installer → reboot-if-needed → exe verify), `BottleArchive` (tar.gz export/import, symlink-safe). Run 35520957256 green both OSes. One CI-caught bug: hardcoded `/bin/tar` missing on macOS → resolve via `/usr/bin/env` (same fix in RuntimeInstaller). Remaining: SwiftUI shell + setup wizard + D3DMetal live proof (all MAC-HANDS).
