...
tags: [builds, productivity, tkinter, focus, deep-work, roadtrip, pomodoro, obsidian, vault-sync, threading, canvas, ambient-sound, animation, slow-roads, endless-cruise, parallax, dark-mode, fullscreen, hud, design-system, spring, fluid, squircle, web, react, nextjs, supabase, vercel, security, csrf, csp, rate-limit, white-noise, brown-noise, pink-noise, cover, landing]
last_updated: "2026-09-10"
confidence: "high"
source: "C:/Users/Vijaykumar/My apps/RoadtripFocus/ (Next.js 16 production — roadtrip-pomodoro, extends flightproductivity.py pattern + Tk single-file web)"
description: "Roadtrip Focus — Next.js 16 production endless cruise (roadtrip-pomodoro, 60fps winding canvas + 4 noise beds + hum, cover, sheet) with Supabase RLS + guest 500 + Vercel + full security (CSP nonce, CSRF double-submit, Upstash-ready rate-limit, email guard). Tk dark-only squircle legacy preserved."
---

# Roadtrip Focus — Cross-Country Focus Timer

> **Repo:** `https://github.com/Anirudh-2810/roadtrip-pomodoro` · **Local:** `C:/Users/Vijaykumar/My apps/RoadtripFocus/` · **Live:** Vercel (`roadtrip-pomodoro`, root `/`) + legacy `docs/roadtrip.html`
> **Stack:** **Next.js 16** (webpack/win32, turbopack/linux) + **Tailwind 4** + **Supabase SSR 0.5.2** + **Resend 4.0.1** + **Zod** + **Canvas2D endless road** (18 stations `pt=1-(1-t)^1.65`, `A1 42-68`, `SCENERY_SPEED 18`, 3 hills, 5 scenery) + **Web Audio** 4s `white/pink/brown/rain + 55/110 Hz hum` + **Security** `CSP nonce` `CSRF` `rate-limit` `HMAC unsubscribe` · **Legacy:** `tkinter` + `legacy/index.html` (68 999B `esm.sh`) + `pywebview` bridge preserved
> **Theme:** Road trip (Coastal Hop → Cross-Country) — endless **Slow Roads** cruise: dark-only midnight neon `bg #09090B surface #121212 accent #10B981` (squircle `r14`, road touches bottom `max y==h`) + **fullscreen HUD** canvas fills viewport, glass HUD
> **Obsidian:** Direct vault writes kept for Tk (`vault_sync.py`); **Cloud:** Supabase `sessions` RLS `auth.uid()=user_id` is prod source, guest `localStorage 500` → `POST /api/guest/claim`

---

## For future agent
**Next.js production build — 2026-09-06 closed.** Next 16 at `Anirudh-2810/roadtrip-pomodoro` now renders the **winding road** natively (`src/components/roadtrip/RoadtripCanvas.tsx` pure Canvas2D, same `pt`/`roadCenter` math as Tk) + `src/lib/audio-roadtrip.ts` 4s Web Audio `brown/pink/white/rain + hum` + `src/components/roadtrip/RoadtripExperience.tsx` (cover `sessionStorage`, `rf_state`/`rf_sessions`, glass HUD, per-row `✕`, fullscreen `canvasWrap`, unified RAF). Prod security: `src/proxy.ts` CSP nonce `script-src 'nonce- strict-dynamic'`, `src/lib/csrf.ts` `__Host-csrf` double-submit, `src/lib/rate-limit.ts` Upstash-ready + `Retry-After`, `src/lib/security.ts` 8KB limit, email `to==own verified` + HMAC unsubscribe. Tk (`roadtrip_focus.py` + `legacy/`) preserved. Cross-links: [[wiki/01-Areas/Self-Dev/productivity/deep-work-attention-economics]], [[wiki/01-Areas/Self-Dev/productivity/focus-minimalism-babauta]], `brain/Roadtrip Focus History`.
...
> **On-the-road fix — reverted 2026-09-03 (blank page):** `On the road` live+delete (`inTransit.length + (isRunning?1:0)`, `live-card`, `sheet-row-del ✕`, `deleteTransit/clearInTransit`) reverted to cover-only baseline `e3b3bd2` due to blank page after `5efe9ba` nested `html` IIFE. `docs/roadtrip.html` back to 61949B cover copy. Retrying with smaller flat diff next.

> **Per-row delete for IN TRANSIT (2026-09-03 flat fix):** Minimal flat diff — no `live-card` IIFE. `On the road` keeps `ROUTES.map 4 READY` `index.html:698` + `logRows.filter(!r.completed).slice(0,3)` `index.html:702` but each `IN TRANSIT` row now has `✕` `sheet-row-del` `24px` `rgba(204,51,51,0.12)` at `8,8` `e.stopPropagation()→deleteTransit(finished_at)` `index.html:702`; row top `paddingRight 28px`. Helpers `deleteTransit` `index.html:484` filters `localStorage rf_sessions` by `finished_at`, `loadLogs()` refresh, closes `detailItem` if deleted. Detail modal `index.html:730` adds `Delete` danger when `!completed`. CSS `.sheet-row{position:relative}` `.sheet-row-del` `index.html:170`. Keeps cover `sessionStorage Let's roll` and 60fps drift, no blank. `docs/roadtrip.html` 63259B resynced.

> **Production PR1 P0 — backup + palette, glass, meter, unified RAF, persistence, notify (2026-09-03):** Backup `backup/prod-PR1-2026-09-03` tags + zip `38782B` + `docs/roadtrip.html` verify 63259B identical before PR1. Visual: `:root --bg #070a0e→#09090B --surface #0f1419→#121212 --surface2 #141b22→#1a1a1e --outline rgba(30,42,51)→rgba(255,255,255,0.08) --accent #00e69a→#10B981 +accent2 #059669` `22`, `header h1/text-shadow 0 0 12px rgba(16,185,129,0.45)` `26`, `canvas-wrap/sheet-map-wrap glow 0 0 20px rgba(16,185,129,0.22)` `30/75`, `sheet-tab.active/btn.primary/pill.active glow rgba(16,185,129,0.25-0.3)` `52/145`, frosted inputs `55/131/165/134/148 → bg rgba(255,255,255,0.05) border white/10 blur16px radius10` + focus `0 0 12px`, meter `139-141 6px linear-gradient accent→accent2 glow 0 0 12px + pct 11px JetBrains` + `time glow 137`. Func: keyboard `Space→Hit/Pause, R→Reset, M→mute, F→fullscreen` `445 INPUT guard`, timer unified `single RAF single clock 321 no setInterval floor(elapsed) + onFinish` + desync `88-100% ease` kept, persistence `rf_state` `ts/intent/route/total/remaining/vol/kind/hum/seed/isRunning/isPaused/startedAt/pausedRef` load/hydrate + save + clear on reset, notification `requestPermission` mount + `onFinish → new Notification + oscillator 880Hz 0.65s chime → remove rf_state`. Copy `docs/roadtrip.html` 67457B resynced. Backup + PR1 P0 production.

> **Gemeini PR3-1 P0 — dashboard glass HUD + state-driven toggle (2026-09-03):** `sheet-footer 67 → bg rgba(28,30,34,0.62) blur12px saturate180% border white/10 shadow 0 12px 40px + inset white/6` + new `hud-main 68 glass 12px 14px rgba(28,30,34,0.72) blur12px border white/10 radius14` flex `hud-time 22px mono 800 accent • hud-progress flex1 bar 6px gradient glow + pct` • `am-icon primary 44px + small Reset` — replaces `timer-row 767` (`time-card 240px + progress-card`) + `controls 771` (`Hit/Pause/Reset 3× btn disabled 0.5`). State: `Idle → single Hit the Road (IconPlay)`, `Active → Pause (IconPause)`, `Paused → Resume (IconPlay)` + Reset `{(isRunning||remaining!==total) && Reset}` + `AnimatePresence mode wait y6 0.18` + `handleHitRoad guard if(isRunning) return` `687`, phase stays `Ready/On the road` `708`. Keeps cover + per-row delete + slate/emerald + RAF + rf_state + notify. `quick/sound/quote` stay below `hud-main`; `sheet-wrap` overflow intact. `docs/roadtrip.html` 68983B.

> **Depth without neon — remove emerald underglow (2026-09-03):** Keep `accent #10B981` solid color, strip all `0 0 20px/12px rgba(16,185,129,0.15-0.45)` halos → black depth only: `header h1 26 / .time 137 text-shadow none`, `.canvas-wrap 30 border rgba(16,185,129,0.18)→rgba(255,255,255,0.08) shadow 0 0 20px rgba(16,185,129,0.22) → 0 24px 48px rgba(0,0,0,0.55)`, `.sheet-map-wrap 81 same`, `.sheet-tab.active 52 / .pill.active 158 0 2px 8px rgba(16,185,129)→0 2px 6px rgba(0,0,0,0.30)`, `.btn.primary 152 / .am-icon.primary 104 0 0 20px → 0 4px 12px rgba(0,0,0,0.35)`, `.fill 147 0 0 12px/20px → 0 2px 6px rgba(0,0,0,0.25)`, inputs focus `55/138/173 keep 0 0 0 3px rgba(16,185,129,0.22) drop aura`. Keeps `slate #09090B`, frosted, meter `6px gradient + pct`, `hud-main` glass already black depth `30/75/68` — no Canvas `#00e69a line 589` touch. `docs/roadtrip.html` 68712B.

> **Header top-left + map top-to-bottom (B) (2026-09-03):** `header` full-width `text-align:center padding 14px` `25-28 19px 1.4px` → moved inside `sheet-wrap` as `.sheet-title` `padding 14px 14px 10px text-align:left bg var(--surface) border-bottom var(--outline)` `h1 14px 0.8px` + `.phase 11px` (hide `p pick a route…` inside title), `sheet-map-wrap 81 min-height420 → 0 align-self stretch`, `#root flex column` keeps `content-area 45/725 flex1 margin 12px 20px` now first child → `height = 100vh - 24px` vs `100vh - 90px` (gains `~66px` vertical), map `canvas-wrap 82 flex1 margin0` fills `sheet-map-wrap` full height `H*0.28 horizon` stays ratio, `isFullscreen 41 + sheet-wrap display:none 726` already hides title in fullscreen. Mobile `90 42vh` title stacks with sheet. `docs/roadtrip.html` 68999B.

> **Mobile stacked layout (2026-09-10, commit `71b183d`, pushed):** phone showed only half the panel — row layout squeezed canvas to ~0 and trapped timer/sound/guest/pull-over below the fold. Mobile now stacks panel-first + road `38dvh` below, lists capped `40dvh`, no clipping; every change mobile-only (`lg:` pins desktop pixel-identical per user). `tsc` + webpack green. Phone-screenshot check pending.

> **Resume audio flash (2026-09-10, commit `24c0efe`, pushed):** resuming played one frame at full volume → mute → ramp. Cause: `startRoadtripAudio` built the loop at `gain = 1` before the tick's ramp-down applied. Fix: loop starts silent (`gain = 0`), cruise branch claims the true level via `driveVol` within a frame. `tsc` + webpack green. Ear-check pending.

> **Break drive model (2026-09-10, commit `0d8f471`, pushed):** killed the slide-on-ice look — old code froze the world instantly while the car glided right. Now a 4-phase drive model in the RAF tick (`cruise → pull → park → merge`): pull-over freezes the wall clock and integrates distance manually with `speed = 1 − smootherstep(t/4.5s)`, so the car steers onto the edge line WHILE the world decelerates; hum fades with speed (`driveVol`); countdown starts only when settled (user pick); merge ramps `0 → 1` over 3s from wherever the car is (Back-on-road mid-pull works); engine-off click at settle, chime at merge end; dashes derive from `d` so they decelerate too; pull/merge seconds count as trip, only settled park time excluded. `tsc` + webpack green. Eyeball check pending.

> **Break parking polish (2026-09-10, commit `032b684`, pushed):** user screenshot showed parked car off-road in the dark — root cause `shoulderX = edge + 26px`. Now straddles the right edge line (`edge − carW×1.1`, wheels on asphalt, glued on curves). Cinematic transitions per user pick (slower): 4.5s smootherstep pull-over, 3s merge-back, 1s delayed run-resume so the car pulls out first and the road catches up; tween state in `parkAnimRef` (survives RAF re-subscribes), hazards at `parkT>0.6`, bob fades with `(1−parkT)`, merge double-tap guard + reset clears resume timer. `tsc` + webpack green. Eyeball check pending (user screenshots).

> **Mobile batch + break timer (2026-09-10, commits `4d42ec1` + `aa077e2`, pushed):** Backup tag `pre-mobile-fix-2026-09-10` + zip. Phone fixes one-by-one: 16px inputs on mobile (iOS zoom guard in `globals.css` + `text-base sm:`), `app-shell`/`fs-full` `100dvh` with `100vh` fallback, done popup `w-[min(340px,100vw-32px)]` + dvh-capped, 44px touch targets (`touch-44`, play/reset/dock). Break Option A: `🅿 Pull over` → 2/5/10/15 picker → `parkedRef` shoulder-ease + hazard blink + frozen dash, local countdown + fuel bar, silence (pause-path) + engine-off click, auto-return + 660→880 chime, zero `sessions` rows. `tsc` + webpack green ×2. Live verify pending: phone screenshots + 1-min trip (1 row) + break (0 rows).

> **Next.js production — endless road + security hardening (2026-09-06, closed [[roadtrip-focus-production-plan]]):** Migrated **full winding road** into Next without `esm.sh`/`pixi` npm — `src/components/roadtrip/RoadtripCanvas.tsx` (Canvas2D, `persp 1-(1-t)^1.65`, `roadCenter A1 sin(w1 d+p1)+A2 sin(w2 d+p2)` `A1 42-68 A2 14-26 w1 0.028-0.048 w2 0.11-0.17`, 22-band sky+stars `hash(sx,seed) sin(dist*0.02)`, 3 hills `0.22/0.45/0.85 tile 420/360/300`, ribbon `18 stations visible 140 halfFar 42 halfNear 260`, `dash 14/gap 14 phase 0.18` scroll, scenery `spacing 18 kind%10` 5 kinds); `src/lib/audio-roadtrip.ts` 4s `white/pink/brown/rain + hum 55+110 Hz` `ensureRoadtripAudio` live-vol; `src/components/roadtrip/RoadtripExperience.tsx` unified RAF `SCENERY_SPEED 18` `dist 0.14 spring` + `12% ease 0.88-1.0` tail, `rf_state`/`rf_sessions` persistence, cover `sessionStorage`, sheet `On the road/History` + `IN TRANSIT ✕`, glass HUD `Go/Play/Pause` + `R`, km `dist/42`, `Trip Log` overlay export/clear, fullscreen `canvasWrap` + `fs-fallback`, `Space/F/R/M/Esc` keys. **Security (do all):** `src/lib/env.ts` Zod `healthEnv` no-value leak, `src/lib/csrf.ts` `__Host-csrf` double-submit `generateCsrfToken/checkCsrf`, `src/lib/security.ts` `getClientIp/readJsonWithLimit 8KB/sanitizeText/generateNonce`, `src/proxy.ts` per-request `nonce` + `CSP default-src 'self' script-src 'nonce- strict-dynamic' style-src 'unsafe-inline' connect-src *.supabase.co/api.resend.com` + `COOP/CORP` (removes `src/middleware.ts`), `src/lib/rate-limit.ts` hybrid Upstash REST `rl:key:window` `INCR PEXPIRE` + `rateLimitAsync` + `Retry-After`, `src/lib/supabase/server.ts` `httpOnly Secure SameSite=lax` + `createServiceClient` server-only, all mutating routes `POST /api/{sessions,guest/claim,email/session,auth/signup|login,email/preferences}` now `rateLimitAsync IP+user` + `readJsonWithLimit` + `checkCsrf` + `Sanitize`, `next.config.ts` extra `COOP CORP XCPCDP`, `src/app/api/csrf` `GET` issues token, `src/app/api/email/unsubscribe` `GET/POST` HMAC `u:exp:sig` + service `upsert`, `src/app/api/email/preferences` `rateLimit 10/m` + CSRF hidden `_csrf`, `src/app/page.tsx` now renders `RoadtripExperience` (not `PomodoroTimer`), `src/lib/email.ts` `List-Unsubscribe-Post One-Click`, `.env.example` docs `UPSTASH_REDIS_*`+`CSRF_SECRET`. `npx tsc --noEmit` clean, `npm run build -- --webpack` green `25.8s → 9.8s` `18 routes ƒ Proxy`, client grep `SERVICE_ROLE/RESEND` 0 hits, `GET /api/health` returns `supabase/resend/redis` status only.
