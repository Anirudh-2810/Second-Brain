...
> **On-the-road fix — only ongoing + delete in-transit (2026-09-03):** `On the road` pile-up in Image 1 (3 `READY` static `ROUTES` + 3 `IN TRANSIT` abandoned `logRows.filter(!r.completed).slice(0,3)` with no delete, badge `On the road (4)` counting `ROUTES.length`) fixed: badge now `inTransit.length + (isRunning?1:0)` `index.html:715`, tab body removed `ROUTES.map` catalog (routes stay via `quick` pills `25/50/90/Custom` + cover), now renders **Live now** card when `isRunning` (intent→route, `fmt(remaining)`, `km`, `progress%`, live bar `live-fill`, `Pause`/`Cancel→handleReset`) + **in-transit list** `logRows.filter(!r.completed)` full (no `slice(0,3)`), each `sheet-row` with `✕` `sheet-row-del` `24px` `rgba(204,51,51,0.12)` at `top 8 right 8` `e.stopPropagation()→deleteTransit(finished_at)` `index.html:738`; header `In transit (n)` + `Clear all →clearInTransit` keeps `delivered`. Empty `No active trips…` when none. Detail modal adds `Delete` danger when `!detailItem.completed`. Helpers `deleteTransit`/`clearInTransit` filter `localStorage rf_sessions` `index.html:474-496`, `loadLogs()` refresh, close detail if deleted. `docs/roadtrip.html` resynced 65690B.
