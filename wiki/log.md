...
- **2026-09-07 — Roadtrip Focus production auth/env fix**: code changes to `src/lib/env.ts` (`getAppUrl()` — prod requires valid URL, dev fallback only), `src/lib/email.ts` (4 links → `getAppUrl()`), `src/app/api/auth/signup/route.ts` (uses `getAppUrl()`). Build clean (`tsc --noEmit`, `npm run build -- --webpack`). Pending: Vercel env vars + Supabase URL config + redeploy so `GET /api/health` shows `supabase: configured` and signup email link resolves to `https://roadtrip-pomodoro-taupe.vercel.app` instead of `localhost:3000`.
...
### 2026-09-03 � Roadtrip Focus cover � Are you ready to hit the road? ? Let's roll (sessionStorage, intent on cover, only main)

- Web C:/Users/Vijaykumar/My apps/RoadtripFocus/index.html: added full-viewport cover overlay (cover-overlay gba(7,10,14,0.78) + lur 18px over idle-drift canvas, AnimatePresence opacity + card y 18?0 spring 320/26) � eyebrow READY TO ROLL ccent #00e69a, title Are you ready to hit the road?, sub, **intent input on cover** (binds intent state, syncs to sheet sheet-intent, Enter dismisses, utoFocus), CTA Let's roll ? tn primary whileTap 0.97, hint. Dismiss sets sessionStorage rf_cover_dismissed=1 (once per tab) ? setShowCover(false); overlay click outside card also dismisses. Keeps 60fps equestAnimationFrame distRenderRef drift behind blur. Checks: cover CSS index.html:158 / state index.html:233 showCover+dismissCover / JSX index.html:659.
- Vault docs/roadtrip.html resynced via Copy-Item index.html docs/roadtrip.html (61948B) for Pages � no new dirs, single-file esm.sh policy kept.
- Wiki wiki/00-Current-Projects/roadtrip-focus.md: last_updated 2026-09-03, 	ags +cover +landing, appended cover block after desync note (see oadtrip-focus.md:330); no light mode re-added, dark-only kept.
- Git: vault branch main only � master removed per user (see log below). RoadtripFocus repo cover pushed to oadtrip-pomodoro main.
### 2026-09-03 � Roadtrip Focus on-the-road pile-up fix � only ongoing + per-row delete

- Fix Image 1 pile-up: On the road (4) counting ROUTES.length + 3 READY static + 3 IN TRANSIT abandoned slice(0,3) with no delete (Trip Log 12 = 9 delivered + 3 in-transit). Now On the road (logRows.filter(!r.completed).length + (isRunning?1:0){' ) badge RoadtripFocus/index.html:715, tab renders only ongoing: **Live now** card when isRunning (intent?route, mt(remaining), km, progress bar live-fill, Pause/Cancel) + **in-transit list** full (no slice) each row with ? .sheet-row-del at 8,8 e.stopPropagation()?deleteTransit(finished_at) index.html:738. Header In transit (n) + Clear all?clearInTransit. Empty No active trips�. Detail modal adds Delete when in-transit. Helpers deleteTransit/clearInTransit at index.html:474-496 filter localStorage rf_sessions, loadLogs() refresh. ROUTES.map catalog removed from tab (routes via quick pills 25/50/90/Custom + cover intent). Static READY no longer piles. docs/roadtrip.html 65690B resynced, push only main.
...
### 2026-09-03 � Gemeini PR3-1 P0 � dashboard glass HUD + state-driven toggle

- Glass HUD: sheet-footer 67 opaque #121212 ? gba(28,30,34,0.62) blur12px saturate180% border white/10 shadow 0 12px 40px + inset, new hud-main 68 glass gba(28,30,34,0.72) blur12px border white/10 radius14 flex hud-time 22px mono 800 accent + hud-progress flex1 bar 6px gradient + meta km/pct + m-icon primary 44px IconPlay/Pause + small Reset IconReset � replaces 	imer-row 767 (	ime-card 240 + progress-card) + controls 771 (3� btn disabled ghost). State: Idle single Hit, Active Pause+Reset, Paused Resume+Reset {(isRunning||remaining!==total)} + AnimatePresence wait y6 0.18 + handleHitRoad guard 687. Keeps cover, per-row delete, slate/emerald, RAF, rf_state, notify. quick/sound/quote below hud-main. docs/roadtrip.html 68983B.
### 2026-09-03 � Depth without neon � remove emerald underglow, black depth only

- Keep accent #10B981 solid, remove halos: header h1 26 + .time 137 text-shadow none, .canvas-wrap 30 + .sheet-map-wrap 81 border rgba(16,185,129,0.18)->rgba(255,255,255,0.08) shadow 0 0 20px emerald -> 0 24px 48px black + inset white/6, .sheet-tab.active 52/.pill.active 158 -> 0 2px 6px black, .btn.primary 152/.am-icon.primary 104 -> 0 4px 12px black, .fill 147 -> 0 2px 6px black, inputs focus keep thin 0 0 0 3px ring drop aura. Black depth verified, focus rings kept for a11y. docs/roadtrip.html 68712B, main only, cover + per-row delete kept.
### 2026-09-03 — Header top-left + map top-to-bottom (B)
...
### 2026-09-05 — Spanish Ingest: VideoEle (videoele.com) — A1 via VideoEle

- Ingested **VideoEle** free Spanish video course (videoele.com, Agustín Iruela, FEDELE 2025) — fetched Curso.html + menu-A1.html + 3 sample units (Primera clase, Números y letras, En el presente). Course = 4 levels A1/A2/B1/B2, each unidad = 3-6 min video + PDF transcripción + Guía didáctica (Antes/Mientras/Después/Para aprender más) + Soluciones + H5P interactives.
- Created `wiki/01-Areas/Self-Dev/spanish/` (6 pages): `overview.md` (hub + 4-level table + 39-unit A1 catalog + 8-week plan + study loop), `alphabet-and-pronunciation.md` (50 cognates, marcas/personajes pronunciation, alphabet, hola/buenos días greetings + numbers 1-100), `grammar-essentials.md` (artículos/género, hay/es/está, demostrativos, presente regular -ar/-er/-ir + ir/hacer, gustar, ser vs estar, saber vs conocer), `vocabulary-and-phrases.md` (12 themed buckets: barrio/tiendas/cuerpo/comida/familia/casa/etc.), `phrasebook-cheatsheet.md` (1-page printable greetings/numbers/days/time/ser-estar), `practice-and-exercises.md` (Guía 4-step workflow + H5P + 8-week grind + CancionEle). All with YAML frontmatter, `## For future agent` preamble, wikilinks, source attribution.
- Updated `wiki/01-Areas/Self-Dev/INDEX.md` (adds spanish row, tags, Quick Answer for hola/VideoEle), ran `generate-index.py` (348 pages) + `update-graph-colors.py` (62 modules).
...
- Local `C:/Users/Vijaykumar/My apps/RoadtripFocus` is new root: `src/app/{page,layout,login,signup,dashboard,settings}` + `src/app/api/{auth/{signup,login,logout},sessions,guest/claim,email/{session,preferences},health}` + `src/components/timer/PomodoroTimer.tsx` (presets 25/5,50/10,15/3+custom mm:ss, intent, Worker fallback, beep 880Hz, Notification, guest save + POST /api/sessions + /api/email/session) + `src/lib/{validation,guest,rate-limit,email,auth}` + `src/lib/supabase/{client,server,middleware}` + `next.config.ts` headers + `supabase/migrations/001_init.sql` (sessions/email_preferences/email_logs with RLS auth.uid()=user_id, pg_cron example).
- Build: `npm install` 393 packages, `npx tsc --noEmit` clean, `next build --webpack` 24.4s compiled successfully 16 routes (3 workers), Turbopack wasm on win32, `middleware` deprecation warning kept.
- Auth: Zod + react-hook-form on `/signup` `/login`, rate limit 5/h & 10/15m via `lib/rate-limit.ts:1`, Supabase `auth.signUp/signIn`, httpOnly cookies via SSR.
- Guest: `Continue without signup →` on `/` and `/signup`/`/login` — `lib/guest.ts:1` IndexedDB/localStorage `guest_sessions` 500 cap + `guest_id`, banner `Guest • Sync to save →`, claim via `POST /api/guest/claim` (1/5m, 500 max, chunk 100) then `clearGuestSessions`, no backfill of per-session emails.
- Email a) auto per completed Pomodoro: `POST /api/email/session` 5/min, enforce `to` == own verified email, Resend `sendSessionEmail` with midnight card + `List-Unsubscribe`, inserts `email_logs`. b) digest: `email_preferences` toggle (daily 22:00 IST + weekly) + `pg_cron` example in migration, `sendDigestEmail` ready, `GET /api/health` shows config.
...
### 2026-09-06 � Roadtrip Focus ? Next.js Production (endless road + full security, closed plan)

- **Closed** wiki/00-Current-Projects/roadtrip-focus-production-plan.md (open ? closed 2026-09-06, user �do all�). Migrated full winding-road endless cruise from single-file RoadtripFocus/index.html (68 999B esm.sh) into **Next.js 16** production Anirudh-2810/roadtrip-pomodoro at C:/Users/Vijaykumar/My apps/RoadtripFocus: src/components/roadtrip/RoadtripCanvas.tsx (Canvas2D, pt=1-(1-t)^1.65, oadCenter A1 42-68 A2 14-26 w1 0.028-0.048 w2 0.11-0.17, 22-band sky+stars, 3 hills  .22/0.45/0.85, ribbon 18 stations, dash 14/gap 14 phase 0.18, 5 scenery kinds, mini car), src/lib/audio-roadtrip.ts (4s white/pink/brown/rain + 55/110 Hz hum Web Audio loop, live-vol), src/components/roadtrip/RoadtripExperience.tsx (cover f_cover_dismissed, f_state/f_sessions persistence, sheet On the road/History + IN TRANSIT ?, HUD glass Go/Play/Pause, km dist/42, Trip Log export/clear, fullscreen canvasWrap fs-fallback, unified RAF SCENERY_SPEED 18  .14 spring + 12% ease 0.88-1.0 tail).
- **Security (do all):** src/lib/env.ts Zod healthEnv (no values), src/lib/csrf.ts __Host-csrf double-submit generateCsrfToken/checkCsrf, src/lib/security.ts getClientIp/readJsonWithLimit 8KB/sanitize/generateNonce, src/proxy.ts per-request 
once + strict CSP default-src 'self' script-src 'nonce- strict-dynamic' + COOP/CORP (replaced src/middleware.ts � Next 16 requires proxy.ts only), src/lib/rate-limit.ts hybrid Upstash Redis REST (l:key:window INCR PEXPIRE) + ateLimitAsync fallback in-mem + Retry-After, src/lib/supabase/server.ts httpOnly Secure SameSite=lax + createServiceClient server-only, all mutating POST /api/{sessions,guest/claim,email/session,auth/signup|login,email/preferences} now ateLimitAsync IP+user + eadJsonWithLimit + checkCsrf + sanitize, 
ext.config.ts extra COOP CORP XCPCDP, new routes GET /api/csrf + GET/POST /api/email/unsubscribe HMAC u:exp:sig, prefs 10/m + hidden _csrf, page now renders RoadtripExperience as hero, email List-Unsubscribe-Post One-Click, .env.example docs UPSTASH_REDIS_* + CSRF_SECRET.
- **Verification:** 
px tsc --noEmit clean (fixed unknown?ReactNode at RoadtripExperience.tsx:372), 
pm run build -- --webpack green 25.8s ? 9.8s after middleware.ts removal (18 routes � Proxy, TypeScript clean), client bundle grep SERVICE_ROLE/RESEND 0 hits, GET /api/health leaks no env (shows supabase/resend/redis statuses). Road math identical to index.html:570 hills+ribbon+dashes+scenery+car; manual eyeball still required (agent cannot see rendered 60fps).
- **Vault:** updated wiki/00-Current-Projects/roadtrip-focus.md (frontmatter last_updated 2026-09-06, tags +
extjs/supabase/security, repo/stack/Obsidian + For-future-agent + new > **Next.js production � endless road + security hardening** block), closed plan file, to run python .scripts/generate-index.py + update-graph-colors.py, commit+push.

### 2026-09-08 — Roadtrip confirm-email localhost fix (deployed)
- **Symptom:** Supabase confirm-account link went to `http://localhost:3000` for every user.
- **Root cause:** prod was serving a pre-2026-09-07 bundle with `localhost:3000` baked in at build time (`NEXT_PUBLIC_*` inlines at build; all prod deploys were 2d old). Vercel Production `NEXT_PUBLIC_APP_URL` already correct (`https://roadtrip-pomodoro-taupe.vercel.app`, verified via redacted `vercel env pull`); local `.env.local` correctly stays `localhost:3000`.
- **Code (roadtrip-pomodoro `095ec3d`, pushed → Vercel auto-deploy Ready):** `src/lib/env.ts` `getAppUrl()` fail-closed in prod (throws instead of localhost fallback); signup + all Resend links via `getAppUrl()`; new `scripts/check-app-url.js` + `prebuild` guard fails Vercel production builds on localhost/missing APP_URL (verified fail + pass paths). Local `tsc --noEmit` clean, `npm run build -- --webpack` green (18 routes incl. `/auth/callback`).
- **Verified live:** `GET /api/health` → `supabase: configured`; `/auth/callback?next=/dashboard` redirects (new bundle serving). Old mails keep localhost links (baked at send time — unfixable retroactively).
- **Still manual (needs user, ~2 min):** Supabase Dashboard → Auth → URL Configuration: Site URL → `https://roadtrip-pomodoro-taupe.vercel.app`, Redirect allowlist add prod `/**` (keep `http://localhost:3000/**`), confirm Email Templates use `{{ .ConfirmationURL }}`; then fresh signup test → link host must be prod. Observed side-note: `/api/health` reports `email: missing RESEND_API_KEY` at runtime despite Vercel listing the secret — verify its value in dashboard (out of scope for this fix).

### 2026-09-08 — Roadtrip guest dashboard/settings + header cleanup (deployed)
- **Was:** logged-out `/dashboard` + `/settings` both `redirect("/login")` — every protected page showed the same login form. Now: `/dashboard` renders new client `src/components/dashboard/GuestDashboard.tsx` (browser-local `guest_sessions` + `guestStats()`, same stat-card style, signup CTA, zero API calls); `/settings` renders account explainer + Create account/Log in CTAs (prefs form stays authed-only — emails are account-side). Authed paths unchanged, no routes/migrations, RLS untouched.
- **Header:** removed `· production` suffix after Roadtrip Pomodoro (`layout.tsx`).
- **Verify:** local `tsc` clean + webpack build green; pushed `30ea87f` → Vercel Production Ready; live logged-out fetch of `/dashboard` (guest mode banner + stats, no login redirect) and `/settings` (explainer) confirmed; header has no production tag.

### 2026-09-08 — Roadtrip duplicates + UTC times + cloud save (deployed)
- **Duplicates:** `onFinish` double-fired via RAF stale-closure race → every trip saved twice (Delivered, guest store, server POST, email). Fixed with `finishedRunRef` one-fire guard per run. Backstops: migration `003_idempotency.sql` (unique `(user_id,started_at)` index — verified live — + missing owner-UPDATE policy), `POST /api/sessions` + guest-claim upsert `ignoreDuplicates`, claim skips rows already on server, client prunes guest row on POST success. Reset-path incomplete rows now POST when authed (no email for breaks).
...
### 2026-09-12 — Ingest: Module-IV Projection of Solids (solid work part)
- **Sources read (not just cataloged):** `Semester 1/Drawing/Module 3 - Projection of Solids.pdf` (36 slides, Parag Sarode — vocab pp 6-16, position bank Q1-Q19 pp 17-34, text-extracted) + `Section and development of solids.pdf` (14 slides, figures-only — parallel-line/radial-line methods + prism/cylinder/cone developments confirmed, no extractable text).
- **Page upgraded:** `engineering-drawing/development-of-surfaces.md` (7.9 → 12.7 KB, no split needed) — new §0 syllabus scope (4.1/4.2, spheres/hollow/composite excluded), prism/pyramid edge vocab + `3n` rule + point-naming convention, axis-position catalog table (⊥HP / ⊥VP / inclined-HP Q8-Q14 / inclined-VP Q15-Q19 incl. cube-diagonal Q19), direct-vs-indirect angle note, Q14 four-wordings note, Section-PDF figures-only caveat, Sources section with raw-source wikilinks + Dubey reference. Frontmatter: course code from slide header (216U06C105, Section slides say 111U06C105 — TBC), confidence medium → high.
- **Catalog:** `wiki/index.md` Engineering Drawing block extended 3 → 7 pages (all ED pages now listed). Domain INDEX already covers the module (no new page, no INDEX change).
- **Gaps flagged in-note:** both PDFs' figures need visual verify (AutoCAD/sketch session); source skips Q7 numbering; triangulation method in neither PDF's text (still TBC).

### 2026-09-12 — Ingest: SolidWorks playlists (154 videos → 26-page module)
- **Sources:** PL1 `SolidWorks Surfacing` (102 vids, CAD CAM Tutorial) + PL2 `Advanced SolidWorks Exercises` (52 vids, DesignWithAjay, ~33 unique after link/timelapse/trailer dupes). Raw dump in `raw-sources/solidworks/`: playlist catalogs with video IDs, 154-row `catalog.csv` (per-video status), 15 transcripts in `transcripts/`.
- **Fetch caveat:** youtube-transcript-api + yt-dlp timedtext hit HTTP 429 mid-run — 12 primers + 3 builds transcribed; all videos confirmed auto-captioned, so bulk retry with `--sleep-subtitles` in a later session will complete the raw set. catalog.csv tracks who still needs fetching.
- **Pages (all under `engineering-drawing/solidworks/`, beginner-first, 3–4k words each, Mermaid flowchart per feature page):** hub INDEX + setup + sketch + part/assembly/drawing workflow; extrude/revolve/sweep + loft/boundary + dress-up combos; surfacing methodology + lofted/boundary deep + fill/knit/trim/thicken + utilities/troubleshooting; PL1 builds (exercises, vessels, electronics, tools, showcase, organic); PL2 machines (gearbox fundamentals, helical/spur, bevel/planetary, shredders, conveyors, presses + drone #309); capstone (flowcharts-master, cheatsheet, 6 project briefs with validation + SimulationXpress/Motion ladders).
- **Interview material cut per user (round is over):** no viva/CSWA page; drone content kept as pure machine-design.
- **Catalog:** Engineering INDEX ED row 6 → 33 pages with solidworks/ pointer; `wiki/index.md` ED block + hub link. Dashboard + graph regenerated.
- **Verify in-app (user-side):** every page ends with a SolidWorks checklist; command paths are 2021–2026-era (press `S` if a button moved).
...
### 2026-09-18 - Ingest: SPM SEM1 work drive (24 files, detailed)

- **Extracted all 24 files** under `raw-sources/SPM SEM1 work/` to text (pptx via python-pptx, pdf via PyMuPDF, docx via python-docx) in `thinking/spm-sem1-extract/`; 4 parallel digests (M1 PPTs, M2 PPTs, practice+quiz, expts+academic).
- **New pages (SPM/, 6):** `spm-module1-faculty-companion.md` (M1 PPT deltas: SI example, largest-of-3 chain, Bohm-Jacopini, error trilogy, guards, const-vs-define, conversion demo) + `spm-module2-faculty-companion.md` (M2 PPT deltas: flag/counting loops, docs demo, calculator guard, fall-through grouping, loop-choice tables, infinite-loop fixes) + `spm-practice-bank-module1.md` (M1 Easy 15 + Difficult 15 verbatim + worked answers) + `spm-practice-bank-module2.md` (M2 Easy 15 + Difficult 15, M2-constructs-only + sketches) + `spm-quiz-bank.md` (both 15-mark timed sets verbatim + worked answers) + `spm-lab-exp1-exp2-guides.md` (EXP1+EXP2 write-ups with tasks + post-lab Q&A + MSYS2/GCC/CodeBlocks appendix).
- **Academic Data verdict:** all 5 PDFs duplicate existing wiki substance (syllabus/lesson-plan/lab-CA/ESE-pattern) - only admin verbatim (approval codes, cover fields, signature footers, book editions) is new; not ingested as pages.
- **Catalog:** Engineering INDEX SPM row 14 -> 20 + 6 new page-map rows + 3 Quick-Answers lines; `wiki/index.md` Semester 1 Concepts extended with Sem-1 drive line. Dashboard + graph regenerated.

### 2026-09-19 — Ingest: Team Onyx Aerodynamics & Avionics PDF (Round-1 test prep)
- **Source:** `[[raw-sources/Aerodynamics_and_Avionics_Study_Material.pdf]]` (16 sections, 15 pp) + in-chat test notice (40 MCQs: 15 math+physics, 15 aptitude, 10 aero; batches 21/22 Sept 6 PM A020).
- **New module `wiki/01-Areas/Engineering/aeromodelling/` (5 pages):** `INDEX.md` hub (test pattern + 2-day reading order) + `aerodynamics-foundations.md` (4 forces, Bernoulli eq + Coanda + Newton-3rd downwash, Equal-Transit myth, bluff vs streamlined, CD split) + `wings-controls.md` (airfoil terms, symmetric vs cambered, 5 planforms, 5 mountings, dihedral/anhedral effect, AR = span^2/area, AoA/stall chain, aileron/elevator/rudder axes) + `avionics-rc-stack.md` (prop diameter×pitch, brushed vs brushless, kV = rpm/V with 1800×12 worked, LiPo 3.7V/cell + 55 A C-rating worked, ESC headroom rule, 2.4 GHz Tx/Rx, servo torque + 60° speed) + `team-onyx-round1-revision.md` (rapid-fire 18 facts, 15 mock aero MCQs with answers, math/physics + aptitude checklists linked to existing formula sheets).
- **Catalog:** Engineering INDEX aeromodelling row; dashboard + graph regen + commit/push.

### 2026-09-19 — Aeromodelling readability pass (simpler + deeper)
- Rewrote all 4 aeromodelling content pages in plain language per user request: short sentences, daily-life examples (spoon/umbrella/hand-out-of-car/bicycle-gears), worked numbers kept, added memory tricks + MCQ trap lines per section.
- `aerodynamics-foundations.md` (85 lines, forces table + Bernoulli/Coanda/Newton split + Equal-Transit + bluff detail); `wings-controls.md` (planform/mounting/dihedral/AR/stall/controls with remember-lines); `avionics-rc-stack.md` (prop/motor/battery/ESC/Tx/Rx/servo with chain overview); `team-onyx-round1-revision.md` (20-line rapid-fire + 15 mocks + math/physics/aptitude checklists + exam traps).

### 2026-09-19 — Aeromodelling sample paper 01 (hard, full 40-Q)
- New page `aeromodelling/team-onyx-sample-paper-01.md`: 40 MCQs with 4 options each (A: 15 math+physics incl. bomb-drop projectile, Bernoulli 1200 Pa, 3-4-5 resultant, determinant, probability 0.145, work 300 J; B: 15 aptitude incl. headwind/tailwind legs, harmonic-mean avg speed, work, series 42, coding, relations; C: 10 aero applications incl. climb+accelerate, AR 12 vs 1.5, stall chain, jammed aileron, 7×6 interceptor, 19980 rpm, 6-min endurance, 30–35 A ESC, anhedral fix, protocol mismatch) + answer grid + reasons. Linked from aeromodelling INDEX (now 6 pages).
...
### 2026-09-20 — Spanish clase 2026-09-20 (16 chat screenshots → new note)
- New page `spanish/clase-2026-09-20-a1-ser-descripciones-familia-genero.md`: SER full table (soy/eres/es/somos/sois/son + matching game), 2 fill-in drills with answers (Chile/españoles, Guatemala, informático, morena/ojos azules, Marilyn rubia, Bieber joven, Bardem/Cruz españoles, Bélgica), descripciones adjective bank EN-translated (core 18 + extended ~30, TBC flags) + opposites (guapo-feo, joven-viejo, grande-pequeño, delgado-gordo, rápido-lento, alto-bajo, fuerte-débil) + worksheet matches (ochenta años→mayor, calle→pobre, dinero→rico, Papá Noel→gordo), gender rules (el libro/la mesa, profesor→profesora, pavo→pava, alto→alta), plural recap (libros/sofás, hoteles/japoneses, esquís-esquíes/marroquís-marroquíes, crisis), familia tree (abuelo/padre/hijo/hermano/tío/primo). Supersedes clase-2026-09-19. Linked from spanish overview. Built from thumbnails — low-confidence items marked (TBC), LearningApps pairs gist-only.
...
### 2026-09-24 - Resume final ingest + vibe-code provenance pass (PII-safe, no DOCX touched)
- Ingested submitted Desktop resume into thinking/2026-09-23-team-onyx-resume.md + wiki/01-Areas/Business/careers/team-onyx-application.md (email/phone/address redacted; raw-sources copy noted stale). Owner verdicts locked: Aero-first prefs, no JS/cert rows, co-curricular as-is, style kept.
- Provenance: stock-agent/ x8 flagged confidence low + interview-prep-guide deprecated as prep (correction note); soft AI-assisted flags on pomodoro-web/ x4, roadtrip-focus-production-plan, projects/handsens101, inventory-system; Builds INDEX rows tagged + recruiter pointer fixed. stock-predictor/neural-engine untouched (no verdict).

### 2026-09-24 - Team Onyx 6-PDF ingest closed + read-before-sleep quick revision
- Coverage: all 6 raw-sources PDFs verified against their notes (no re-extract needed — de72c85 ingest stood): ESCs → esc-deep-dive; Propulsion → propulsion-system-deep-dive; Workshop PPT + WORKSHOP PPT 2021 → workshop-ppts-merged; Juniors → airplanes-for-juniors-foundations; Study Material → Round-1 pages.
- New: wiki/01-Areas/Engineering/aeromodelling/team-onyx-quick-revision.md — 30 numbered plain-word facts + everyday examples (19th-note style) across 5 sections (ESC, propulsion, build flow, juniors colour, Round-1 numbers); linked to all deep dives + mock.
- INDEX.md: added 6-PDF coverage-map table + quick-revision row; interview order now starts at step 0 (read tonight, sleep).

### 2026-09-24 - Team Onyx interview notes rebuilt as PROPER teaching pages (owner: cheat sheet had no understanding)
- Rewrote all 4 interview notes in the 19th-note teaching style (tables, daily-life examples, worked numbers, memory tricks, INTERVIEW trap lines, self-checks): esc-deep-dive.md (PPM, MOSFET vs resistor, BEC family, headroom), propulsion-system-deep-dive.md (B-E-M-P chain, outrunner/inrunner, prop numbers, kV + LiPo worked examples, matching rules), workshop-ppts-merged.md (history, classification axes, 5-step design process, wing numbers, final-connections checklist, 2021 deltas), airplanes-for-juniors-foundations.md (dynamic-pressure math, vortices, drag ladder, turn balance, decalage, tube truss).
- INDEX.md: new "How to study (read this first)" protocol (proper notes → cover-recall → self-check → cheat sheet → mock); quick-revision header now points beginners to the proper notes first.

### 2026-09-24 — Ingest: ED textbooks (Bhatt + Dubey) + matrices class scan
- New engineering-math/matrices-class-notes-2026-09-24.md: verbatim-class distillation of `raw-sources/DocScanner 24 Sept 2026 2-46 pm.pdf` (5 pp handwritten, image-only — transcribed via rendered PNGs): Hermitian/Skew-Hermitian defs + split table + exam answer method, orthogonal/unitary quick defs, echelon-rank example (rank 2), homogeneous rule (r=n zero-only / r<n n-r solutions), augmented consistency example (ρ(AB)≠ρ(A) → inconsistent), linear dependence one-liner, Gauss-Jacobi opening iteration (NEW vs Module-1, diagonal-dominance condition, first iterates 0.85/−0.9/1.25). Handwriting-ambiguous digits flagged TBC. Linked from module-1-matrices tail.
