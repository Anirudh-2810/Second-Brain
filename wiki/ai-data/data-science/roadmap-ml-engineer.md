...
**Branch A: GenAI/LLM (highest 2026 premium)** — prompt engineering → embeddings + vector search → RAG pipelines → fine-tuning (LoRA/PEFT) → agents/tool use. You already own a flagship artifact here: the RAG business-brain ([[builds/retrieval-agent/overview]]). India fresher band with these skills: ₹15–25 LPA `(as of 2026)`.
