...
- **2026-09-22 — PowerPoint owner files (`~$*.pptx`) sneak into git.** While a deck is open, Office drops a `~$` lock file beside it; `git add -A` commits it. Fix: `.gitignore` now has `~$*`; always `git status --short` before committing office docs.
